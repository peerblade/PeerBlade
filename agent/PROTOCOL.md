...
`PEERBLADE_MANAGED_INTERFACE`. Other interfaces may be included in read-only
...
failures. WireGuard remains in the kernel with its last applied configuration
when the agent or control plane is unavailable; the agent is not part of the
VPN traffic path.
